...
def _extract_patient_metrics(appointments: List[Appointment]) -> Dict:
    ...
    if appointments:
        ...
        months_active = max(1.0, (now - first_appointment).days / 30.0)
...
def _identify_risk_reason(metrics: Dict) -> str:
    ...
    if metrics["appointments_last_30"] == 0:
        reasons.append("Nenhuma consulta no último mês")
    
    return "; ".join(reasons) if reasons else "Padrão normal de consultas"

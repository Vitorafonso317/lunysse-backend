...
def send_email_request_reject(patient_email: str, patient_name: str, psychologist_name: str):
    ...
    html = f"""
        ...
        <p>Este psicologo nao aceitou sua solicitaçao, Voçe pode escolher outro psicologo para enviar uma nova solicitaçao.</p>
        <p>Obrigado por utilizar nossa plataforma enviado pelo aluno vitor.</p>
    """
...
# =============================
def send_email_appointment_status_change(
    patient_email: str,
    patient_name: str,
    old_status: str,
    new_status: str,
    ...
    html = f"""
        ...
        <p><strong>Data da consulta:</strong> {appointment_date}</p>
        <br>
...
# =============================
# EMAIL: ALTERAÇÃO DE STATUS DO AGENDAMENTO (UPDATE)
# =============================
def send_email_appointment_status_update(
    ...
    """
    Envia e-mail informando status anterior e novo status, junto com data e horário.
    Usa a função genérica send_email e a variável de ambiente EMAIL_SENDER (se existir).
    """
    sender = os.getenv("EMAIL_SENDER") or os.getenv("EMAIL_DOMAIN")
    html = f"""
        <h3>Olá {patient_name},</h3>
        <p>O status do seu agendamento foi atualizado.</p>
        <p><strong>Status anterior:</strong> {old_status}</p>
        ...
        <br>
        <p>Se tiver dúvidas, entre em contato com seu psicólogo.</p>
    """
    return send_email(
        to_email=patient_email,
        subject="Atualização no status do seu agendamento",
        html_content=html,
        sender_email=sender,
        sender_name="Sistema de Agendamentos"
    )
...
def send_email_appointment_status_cancel(
    ...
    """
    Envia e-mail informando que o agendamento foi cancelado.
    appointment_date/time são opcionais (serão incluídos se fornecidos).
    """
    sender = os.getenv("EMAIL_SENDER") or os.getenv("EMAIL_DOMAIN")
    date_block = f"<p><strong>Data:</strong> {appointment_date}</p>" if appointment_date else ""
    time_block = f"<p><strong>Horário:</strong> {appointment_time}</p>" if appointment_time else ""
    html = f"""
        <h3>Olá {patient_name},</h3>
        <p>Seu agendamento foi <strong>cancelado</strong>.</p>
        {date_block}
        {time_block}
        <p>Se deseja reagendar, entre em contato com seu psicólogo ou crie um novo agendamento pela plataforma.</p>
    """
    return send_email(
        to_email=patient_email,
